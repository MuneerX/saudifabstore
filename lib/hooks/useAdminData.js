  ...
  const fetchProducts = useCallback(async (searchFilters = {}) => {
    try {
      ...
      const formattedProducts = productsData.map(product => ({
        ...
        image: product.images?.[0] || product.image || '/images/home/category_grid/container_3.jpeg',
  ...
  const fetchCustomers = useCallback(async (searchFilters = {}) => {
    try {
      ...
      Object.keys(orderCustomerMap).forEach(emailKey => {
        if (!existingEmails.has(emailKey) && emailKey !== 'admin@brooqalkhalij.com' && emailKey !== 'admin@example.com') {
